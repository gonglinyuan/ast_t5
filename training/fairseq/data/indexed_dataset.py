...
try:
    ...
    def deserialize_triplet(b):
        (xyz,) = struct.unpack("<I", b)
        z = xyz & 0x3FFF
        xy = xyz >> 14
        y = xy & 0xF
        x = xy >> 4
    ...
    def deserialize_array(b):
        assert len(b) % 4 == 0
        r = np.zeros(len(b) // 4 * 3 + 1, dtype=np.int64)
        for i in range(0, len(b), 4):
            x, y, z = deserialize_triplet(b[i: i + 4])
            j = (i // 4) * 3
            r[j] = x
            r[j + 1] = y
            r[j + 2] = z
    ...
    class FFRecordDataset(torch.utils.data.Dataset):
        def __init__(self, fname):
            ...
            self.reader = FileReader(os.path.join(dirname, "ffr", basename + ".ffr"))
        ...
        def __getitem__(self, idx):
            return torch.from_numpy(deserialize_array(self.reader.read_one(idx)))

except ImportError:
    FFRecordDataset = None
