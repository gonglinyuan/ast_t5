...
try:
    ...
    class SoftmaxDropoutFast(torch.autograd.Function):
        ...
        def forward(ctx, is_training, heads, inputs, dropout_prob):
            ...
            dropout_results, dropout_mask, softmax_results = fused_softmax_dropout_fast_cuda.forward(
                is_training, heads, inputs, dropout_prob, None)
            if is_training:
                ctx.heads = heads
        ...
        def backward(ctx, grad_output):
            ...
            grad_input = fused_softmax_dropout_fast_cuda.backward(
                heads, grad_output, softmax_results, dropout_mask, dropout_prob)
            return None, None, grad_input, None

    fused_softmax_dropout_func = SoftmaxDropoutFast.apply
except ImportError:
    fused_softmax_dropout_func = None
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            (fused_softmax_dropout_func is None or need_weights)
        ...
        if attn_mask is not None:
            if attn_mask.dim() == attn_weights.dim() - 1:
                attn_mask = attn_mask.unsqueeze(0)
                if self.onnx_trace:
                    attn_mask = attn_mask.repeat(attn_weights.size(0), 1, 1)
            else:
                assert attn_mask.dim() == attn_weights.dim()
        ...
        if fused_softmax_dropout_func is None or need_weights:
            ...
            attn_probs = fused_softmax_dropout_func(
                self.training or self.dropout_module.apply_during_inference,
                self.num_heads,
                attn_weights,
                self.dropout_module.p
