...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("clip-grads"):
                # clip grads
                grad_norm, grad_log = self.clip_grad_norm(self.cfg.optimization.clip_norm)
            ...
            if self.tpu:
                ...
                if self.get_num_updates() % self.cfg.common.log_interval == 0:
                    ...
                    logging_output = self._reduce_and_log_stats(
                        logging_outputs, sample_size, grad_norm, grad_log
                    )
    ...
    def clip_grad_norm(self, clip_norm):
        ...
        if self.cfg.common.verbose_grad_logging is not None:
            grad_log = self.get_verbose_grad_log(agg_norm_fn if should_agg_norm else None)
        else:
            grad_log = None
        grad_norm = self.optimizer.clip_grad_norm(
            clip_norm, aggregate_norm_fn=agg_norm_fn if should_agg_norm else None
        )
        return grad_norm, grad_log

    @torch.no_grad()
    def get_verbose_grad_log(self, aggregate_norm_fn):
        pattern = re.compile(self.cfg.common.verbose_grad_logging)
        ...
        for name, param in self.model.named_parameters():
            ...
            if param.grad is None:
                continue
            key = pattern.match(name)
            if key is None:
                key = "others"
            else:
                key = key.group(0)
                assert key != "others"
            grads[key].append(param.grad.detach())
        grad_log = {}
        for key in grads.keys():
            total_norm = torch.norm(
                torch.stack(
                    [torch.norm(g, p=2, dtype=torch.float32) for g in grads[key]]
                )
            )
            if aggregate_norm_fn is not None:
                total_norm = aggregate_norm_fn(total_norm)
            grad_log[key] = self.optimizer.scale_grad_norm(total_norm)
        return grad_log
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None, grad_log=None):
        ...
        if grad_log is not None:
            for key, value in grad_log.items():
                assert not torch.is_tensor(value) or torch.isfinite(value)
                metrics.log_scalar(f"grad_{key}", value, priority=510, round=3)
