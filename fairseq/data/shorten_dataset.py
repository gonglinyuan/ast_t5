...
class TruncateDataset(BaseWrapperDataset):
    ...
    def __init__(self, dataset, truncation_length, ignore_eos_id=None):
        super().__init__(dataset)
        assert truncation_length is not None
        self.truncation_length = truncation_length
        assert truncation_length >= 1
        self.ignore_eos_id = ignore_eos_id
    ...
    def __getitem__(self, index):
        ...
        if item_len > self.truncation_length:
            if self.ignore_eos_id is not None:
                assert item[-1].item() == self.ignore_eos_id
                item = torch.cat([item[: self.truncation_length - 1], item.new([self.ignore_eos_id])])
            else:
